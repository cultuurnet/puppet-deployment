class deployment::udb3::iis (
  ...
  exec { 'iis-silex-db-install':
    ...
    onlyif    => "test 0 -eq $(mysql --defaults-group-suffix=_iis -s --skip-column-names -e 'select count(table_name) from information_schema.tables where table_schema = \"${db_name}\" and table_name not like \"doctrine_migration_versions\";')",
